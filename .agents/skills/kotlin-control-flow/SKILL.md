...
When a branch first matches a type/value and then checks an extra predicate, use a guard condition (requires the Kotlin version in `gradle/libs.versions.toml` — see [`../../reference/coding-conventions.md`](../../reference/coding-conventions.md) for this project's pinned language level):
...
For a `when` expression over a closed domain (a `sealed class`/`sealed interface`, per this project's [modeling data & state convention](../../reference/coding-conventions.md)), handle every case explicitly. Do not add `else` only to quiet the compiler.
...
Use `else` when the domain is open: strings from an external source, integer status codes, unknown values, or a deliberate fallback/logging path.
...
After reshaping, verify that every branch still has the narrowed type available where it is used. If the rewrite forces `as`, `!!` (never use `!!` in this project — see [coding-conventions.md](../../reference/coding-conventions.md)), temporary mutable vars, or duplicated casts, keep the original shape or choose a smaller refactor.
...
- Do not introduce guard conditions if the project's pinned Kotlin language level does not support them (check `apiVersion`/`languageVersion` in `build.gradle.kts`).
...
- [`../../reference/coding-conventions.md`](../../reference/coding-conventions.md) — this project's naming, null-safety, and data-modeling conventions.
