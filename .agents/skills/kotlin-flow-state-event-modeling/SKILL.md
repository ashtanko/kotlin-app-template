...
If absence, loading, or error is a real state, model it explicitly (`User?`, `sealed interface UiState`, `Result`, etc. — see this project's [data/state modeling convention](../../reference/coding-conventions.md)). The bug is a fake domain value masquerading as real data, not every initial value.
...
fun preferences(): StateFlow<Prefs> =
    repo.prefsFlow.stateIn(scope, SharingStarted.Eagerly, Prefs.Default)
```

Every call to `preferences()` launches a fresh coroutine on `scope` that never completes. Performance dies fast under repeated reads.

```kotlin
// ✅ GOOD — one shared instance, computed once
val preferences: StateFlow<Prefs> =
    repo.prefsFlow.stateIn(ownedScope, SharingStarted.Eagerly, Prefs.Default)
```
...
val name: StateFlow<String> = userState
    .map { it.name }
    .stateIn(ownedScope, SharingStarted.Eagerly, userState.value.name)
```
...
| Cold stream, one consumer per collection | Plain `Flow` (see `DataProcessor.processDataStream`) |
...
| `MutableSharedFlow<Event>` for single-consumer notification | Lossy default event stream | Consider `Channel(BUFFERED).receiveAsFlow()` |
...
| "I'll use a sentinel until real data loads" | Consumers treat it as real domain; prefer explicit state modeling or phasing. |
...
- [`../../reference/testing.md`](../../reference/testing.md) — asserting `Flow` behavior with Turbine.
