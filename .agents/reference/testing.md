...
Mirror [`DataProcessorTest.kt`](../../src/test/kotlin/dev/shtanko/template/DataProcessorTest.kt) and
[`ExampleTest.kt`](../../src/test/kotlin/dev/shtanko/template/ExampleTest.kt).

## Principle

Test the smallest contract that proves the behavior. This is a single-module JVM project, so there
is no test-layer selection to make — everything lives in `src/test/kotlin` and runs on plain JUnit 5.
